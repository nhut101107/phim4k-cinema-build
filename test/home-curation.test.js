...
test('home keeps a larger useful catalogue for browsing',()=>{
  const list=Array.from({length:150},(_,index)=>movie(`movie-${index}`,{year:2026,type:index%2?'series':'single',modified:{time:`2026-08-${String(index%28+1).padStart(2,'0')}T00:00:00Z`}}));
  ...
  assert.equal(data.sections.find(section=>section.id==='latest').items.length,120);
});
