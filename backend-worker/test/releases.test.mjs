...
test('public installer routes stream the fixed 3.53 files without exposing a redirect', async () => {
  ...
  try {
    ...
    assert.equal(response.headers.get('content-disposition'), 'attachment; filename="4K-Cinema-iOS-3.53-unsigned.ipa"');
    ...
    assert.match(requests[0].url, /4K-Cinema-iOS-3\.53-unsigned\.ipa$/);
  } finally {
