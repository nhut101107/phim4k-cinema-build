...
const CORS_HEADERS = {
  ...
  "access-control-allow-headers": "content-type, x-license-key, x-telegram-id, x-app-version",
...
function keyPayload(record, isAdminUser = false) {
  ...
  return {
    ...
    assignedTelegramId: record.assigned_telegram_id || "",
    telegramId: record.activated_telegram_id || "",
    deviceId: isAdminUser ? (record.device_id || "") : undefined,
...
async function listKeys(request, env) {
  ...
  return json({
    ...
    stats: {
      ...
      expired: keys.filter((item) => item.expiresAt && isExpired(item.expiresAt)).length,
    },
  });
}
...
async function createKey(request, env) {
  ...
  try {
    ...
    ).bind(key, String(body.plan || "STANDARD").trim().slice(0, 64) || "STANDARD", plusDays(null, durationDays), normalizeId(body.assignedTelegramId), createdAt, createdAt).run();
  } catch (_error) {
...
async function updateKey(request, env, operation) {
  ...
  if (operation === "renew") {
    ...
    await env.DB.prepare("UPDATE license_keys SET assigned_telegram_id = ?, activated_telegram_id = NULL, updated_at = ? WHERE license_key = ?").bind(normalizeId(body.newTelegramId), timestamp, key).run();
...
async function listUsers(request, env) {
  ...
  const missing = dbUnavailable(env);
  if (missing) return missing;
  const rows = await env.DB.prepare("SELECT * FROM license_keys WHERE activated_telegram_id IS NOT NULL OR assigned_telegram_id IS NOT NULL ORDER BY updated_at DESC").all();
...
async function handleLogs(request, env) {
  ...
  const result = await env.DB.prepare("SELECT * FROM audit_logs ORDER BY id DESC LIMIT ?").bind(limit).all();
